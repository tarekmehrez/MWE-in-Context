...
# 3- reformat text file to remove special chars, and to be lowercased
$JAVA --reformat-text corpus/all.txt

# 4- create the final text file with mwes as phrases (_ separated) to be fed into word2vec and glove
$JAVA --construct-phrases corpus/all.txt.out corpus/IOB/wiki50_distilled.iob.out



# 5- produce vectors using word2vec (for skipgram and cbow models) https://code.google.com/p/word2vec/
...
# 6- create word clusters using the word2vec clustering tool
...
# 7- produce vectors using glove nlp.stanford.edu/projects/glove/
mkdir $glove/results
...
# 8- Calculate distances between MWE vectors and separate tokens' vectors, 
